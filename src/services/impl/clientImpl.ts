...
export class ClientServiceImpl implements IClientService {
  ...
  async createOrUpdate(client: Client, siret: string): Promise<Client> {
    ...
    try {
      ...
      throw Error(`Error during ${isNew ? 'creating' : 'editing'} new client`);
    }
  }  
  async findAllBySiret(siret: string): Promise<Client[]> {
    try {
      const response = await Webservice.getInstance().get(`${ClientServiceImpl.CLIENT_PATH}/${siret}`);
      return response.data;
    } catch (error) {
  ...
  async deleteById(id: number): Promise<string> {
    try {
      await Webservice.getInstance().delete(`${ClientServiceImpl.CLIENT_PATH}/${id}`);
